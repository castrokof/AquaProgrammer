...
class FacturacionMasivaController extends Controller
{
    ...
    /**
     * Obtener lecturas no normales para selección manual
     */
    public function lecturasNoNormales(Request $request)
    {
        $request->validate([
            'periodo_lectura_id' => 'required|exists:periodos_lectura,id',
        ]);
        ...
        $lecturasFiltradas = $lecturas->filter(function($l) {
            return !Factura::where('suscriptor', $l->Suscriptor)
                ->where('periodo_lectura_id', $l->periodo_lectura_id)
                ->exists();
        });

        return response()->json([
            'ok' => true,
            'lecturas' => $lecturasFiltradas->values(),
        ]);
    }

    /**
     * Facturar lecturas seleccionadas manualmente
     */
        ...
        try {
            foreach ($request->lecturas as $lecturaData) {
                ...
                try {
                    ...
                    // Usar datos enviados o calcular
                    ...
                    $calculo['observaciones'] = 'Facturación manual - Lectura seleccionada: ' . ($lectura->Critica ?? 'SIN_CRITICA');
                    $calculo['usuario_id'] = auth()->id();
                    ...
                    Log::error('Error en facturación seleccionada: ' . $e->getMessage(), [
            ...
            return response()->json([
                ...
                'resultado' => $resultado,
                'mensaje' => 'Proceso completado. ' . 
                    $resultado['facturadas_automaticas'] . ' facturadas, ' .
                    $resultado['errores'] . ' errores.',
            ]);

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Error grave en facturación seleccionada: ' . $e->getMessage());

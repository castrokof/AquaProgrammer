...
@section('scripts')
<script src="{{asset("assets/pages/scripts/admin/menu-rol/index_menu.js")}}" type="text/javascript"></script>
@endsection
            ...
            @foreach ($menus as $key => $menu)
                ...
                @endif
                    ...
                    @foreach ($menu["submenu"] as $key => $hijo)
                        <tr>
                            <td class="pl-30"><i class="fa fa-arrows-right"></i> {{$hijo["nombre"]}}</td>
                            @foreach ($rols as $id =>$nombre)
                            <td class="text-center">
                            <input
                            type="checkbox"
                    ...
                    @foreach ($hijo["submenu"] as $key => $hijo2)
                        <tr>
                            <td class="pl-40"><i class="fa fa-arrows-right"></i> {{$hijo2["nombre"]}}</td>
                            @foreach ($rols as $id =>$nombre)
                            <td class="text-center">
                            <input
                            type="checkbox"
                        ...
                        <tr>
                            <td class="pl-50"><i class="fa fa-arrows-right"></i> {{$hijo3["nombre"]}}</td>
                            @foreach ($rols as $id =>$nombre)
                            <td class="text-center">
                            <input
                            type="checkbox"
